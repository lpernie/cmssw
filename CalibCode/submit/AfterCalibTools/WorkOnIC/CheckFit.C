...
void CheckFit( TString File, TString folder , int EBorEE=0, bool Are_pi0_=true ){
  ...
  Float_t EBfit_b0_, EBfit_b1_, EBfit_b2_, EBfit_b3_, EBfit_Bnorm_, EBfit_mean_, EBfit_sigma_, EBfit_Snorm_, EBChisqu_, EBNdof_;
  ...
  Float_t EEfit_b0_, EEfit_b1_, EEfit_b2_, EEfit_b3_, EEfit_Bnorm_, EEfit_mean_, EEfit_sigma_, EEfit_Snorm_, EEChisqu_, EENdof_;
  ...
  for( int EBEE=0; EBEE<2; EBEE++ ){
	...
	//Get Variables
	tree->GetEntry(i);
	float fit_b0_, fit_b1_, fit_b2_, fit_b3_, fit_mean_, fit_sigma_, fit_Snorm_, fit_Bnorm_, Chisqu_, Ndof_, ietaX_, iphiY_, iZ_, Index_;
	if(EBEE==0){
	  ...
	  ietaX_=EBieta_; iphiY_=EBiphi_; iZ_=0, Index_=EBhashedIndex_;
	}
	if(EBEE==1){
	  ...
	  ietaX_=EEix_; iphiY_=EEiy_; iZ_=EEzside_; Index_=EEhashedIndex_;
	}
...
cout<<" Coo1BadEE[j]+1, "<< Coo1BadEE[j]+1<<" "<<Coo2BadEE[j]+1<<endl;
